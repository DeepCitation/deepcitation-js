...
function resolveTransition(
  ...
  if (stage === "enter-b") return `transform ${enterStepMs}ms ${BLINK_ENTER_EASING}`;
  if (stage === "steady") {
    const settleMs = Math.max(16, enterTotalMs - enterStepMs);
    return `transform ${settleMs}ms ${BLINK_ENTER_EASING}`;
  }
  return `transform ${exitMs}ms ${BLINK_EXIT_EASING}`;
}
...
function containerTransform(stage: BlinkMotionStage): string {
  ...
  if (stage === "enter-a") return `translate3d(0, ${BLINK_ENTER_Y_A_PX}px, 0)`;
  if (stage === "enter-b") return `translate3d(0, ${BLINK_ENTER_Y_B_PX}px, 0)`;
  if (stage === "exit") {
    return `translate3d(0, ${BLINK_EXIT_Y_PX}px, 0)`;
  }
...
export function getBlinkContainerMotionStyle(
  ...
  return {
    ...
    willChange: stage === "steady" ? undefined : "transform",
  };
}
