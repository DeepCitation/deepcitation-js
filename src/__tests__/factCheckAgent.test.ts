...
describe("createFactCheckAgent", () => {
  ...
  describe("factCheck", () => {
    ...
    it("performs full workflow with file source and inline citations", async () => {
      ...
      mockFetch.mockResolvedValueOnce(
        mockVerifyResponse({
          // The key will be a hash generated by getAllCitationsFromLlmOutput
          // We need to match whatever key the parser produces
        }),
      );
      ...
      // Citations should have been parsed
...
<<<END_CITATION_DATA>>>`;
    ...
    it("correctly computes summary with mixed verification statuses", async () => {
      ...
      // Verification — we need to handle the dynamic keys
      // The verify method groups citations by attachmentId, so it calls verifyAttachment
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: async () => ({
          verifications: {
            // Keys here need to match what getAllCitationsFromLlmOutput generates
            // They are hash-based, but verifyAttachment passes them through
            // We'll check the summary counts instead of exact keys
          },
        }),
      } as Response);
