...
import { getCitationPageNumber, normalizeCitations } from "../parsing/normalizeCitation.js";
...
describe("normalizeCitations", () => {
  it("normalizes cite attributes, ordering, and content", () => {
    ...
    expect(normalized).toBe(
      "Intro <cite file_id='file123' start_page_key='page_number_2_index_0' full_phrase='Hello world' line_ids='1,2,3' reasoning='A \\\"quote\\\"' /> outro",
    );
  });
  ...
  describe("AV citations ordering (timestamps)", () => {
    it("orders AV citation attributes: file_id, full_phrase, timestamps, optional attrs", () => {
      ...
      // Should be ordered: file_id, full_phrase, timestamps, value
      const fileIdIndex = result.indexOf("file_id=");
    ...
    it("handles AV citations with reasoning instead of value", () => {
      ...
      const result = normalizeCitations(input);
      expect(result).toContain("file_id='audio456'");
    ...
    it("expands timestamp ranges into comma-separated values", () => {
      const input = `<cite file_id='vid' full_phrase='test' timestamps='1-5' />`;
      const result = normalizeCitations(input);
  ...
  describe("optional value vs reasoning ordering", () => {
    it("places reasoning before value when both present", () => {
      const input = `<cite file_id='file1' start_page_key='page_1_index_0' full_phrase='phrase' line_ids='1' value='val' reasoning='reason' />`;
      const result = normalizeCitations(input);
    ...
    it("handles only value attribute", () => {
      ...
      const input = `<cite file_id='file1' start_page_key='page_1_index_0' full_phrase='phrase' line_ids='1' value='val123' />`;
      const result = normalizeCitations(input);
    ...
    it("handles only reasoning attribute", () => {
      const input = `<cite file_id='file1' start_page_key='page_1_index_0' full_phrase='phrase' line_ids='1' reasoning='only reason' />`;
      const result = normalizeCitations(input);
  ...
  describe("descending line-id ranges", () => {
    it("expands ascending line-id ranges correctly", () => {
      const input = `<cite file_id='file1' start_page_key='page_1_index_0' full_phrase='phrase' line_ids='3-7' />`;
      const result = normalizeCitations(input);
    ...
    it("handles descending line-id ranges by returning only start value", () => {
      ...
      const input = `<cite file_id='file1' start_page_key='page_1_index_0' full_phrase='phrase' line_ids='10-5' />`;
      const result = normalizeCitations(input);
      expect(result).toContain("line_ids='10'");
    });

    it("handles single line-id", () => {
      const input = `<cite file_id='file1' start_page_key='page_1_index_0' full_phrase='phrase' line_ids='42' />`;
      const result = normalizeCitations(input);
      expect(result).toContain("line_ids='42'");
    });

    it("handles comma-separated line_ids", () => {
      const input = `<cite file_id='file1' start_page_key='page_1_index_0' full_phrase='phrase' line_ids='1,2,5,8' />`;
      const result = normalizeCitations(input);
      expect(result).toContain("line_ids='1,2,5,8'");
    });

    it("handles mixed ranges and individual line_ids", () => {
      const input = `<cite file_id='file1' start_page_key='page_1_index_0' full_phrase='phrase' line_ids='1-3,7,9-11' />`;
      const result = normalizeCitations(input);
  ...
  describe("entity edge cases", () => {
    it("decodes &amp; entity", () => {
      const input = `<cite file_id='file1' start_page_key='page_1_index_0' full_phrase='Tom &amp; Jerry' line_ids='1' />`;
      const result = normalizeCitations(input);
      expect(result).toContain("full_phrase='Tom & Jerry'");
    });

    it("decodes &quot; entity", () => {
      const input = `<cite file_id='file1' start_page_key='page_1_index_0' full_phrase='He said &quot;hello&quot;' line_ids='1' />`;
      const result = normalizeCitations(input);
    ...
    it("decodes &apos; entity", () => {
      const input = `<cite file_id='file1' start_page_key='page_1_index_0' full_phrase='It&apos;s working' line_ids='1' />`;
      const result = normalizeCitations(input);
    ...
    it("decodes &lt; and &gt; entities", () => {
      const input = `<cite file_id='file1' start_page_key='page_1_index_0' full_phrase='5 &lt; 10 &gt; 3' line_ids='1' />`;
      const result = normalizeCitations(input);
      expect(result).toContain("full_phrase='5 < 10 > 3'");
    });

    it("handles already escaped single quotes", () => {
      const input = `<cite file_id='file1' start_page_key='page_1_index_0' full_phrase='It\\'s already escaped' line_ids='1' />`;
      const result = normalizeCitations(input);
    ...
    it("handles double-escaped quotes correctly", () => {
      const input = `<cite file_id='file1' start_page_key='page_1_index_0' full_phrase='It\\\\'s double' line_ids='1' />`;
      const result = normalizeCitations(input);
    ...
    it("removes markdown bold/italic markers from content", () => {
      const input = `<cite file_id='file1' start_page_key='page_1_index_0' full_phrase='This is **bold** and __italic__' line_ids='1' />`;
      const result = normalizeCitations(input);
      expect(result).toContain("full_phrase='This is bold and italic'");
    });

    it("removes asterisks from content", () => {
      const input = `<cite file_id='file1' start_page_key='page_1_index_0' full_phrase='Item * with * stars' line_ids='1' />`;
      const result = normalizeCitations(input);
  ...
  describe("attribute key canonicalization", () => {
    it("converts fullPhrase to full_phrase", () => {
      const input = `<cite file_id='file1' start_page_key='page_1_index_0' fullPhrase='test phrase' line_ids='1' />`;
      const result = normalizeCitations(input);
      expect(result).toContain("full_phrase='test phrase'");
    });

    it("converts lineIds to line_ids", () => {
      const input = `<cite file_id='file1' start_page_key='page_1_index_0' full_phrase='test' lineIds='1,2,3' />`;
      const result = normalizeCitations(input);
      expect(result).toContain("line_ids='1,2,3'");
    });

    it("converts fileID to file_id", () => {
      const input = `<cite fileID='file1' start_page_key='page_1_index_0' full_phrase='test' line_ids='1' />`;
      const result = normalizeCitations(input);
      expect(result).toContain("file_id='file1'");
    });
  });

  describe("newline handling", () => {
    it("flattens newlines to spaces in attribute values", () => {
      const input = `<cite file_id='file1' start_page_key='page_1_index_0' full_phrase='line one
line two
line three' line_ids='1' />`;
  ...
  describe("attribute reordering for document citations", () => {
    it("orders document citation attributes: file_id, start_page_key, full_phrase, line_ids", () => {
      // Use properly formed input that the regex can parse
      const input = `<cite file_id='doc123' start_page_key='page_5_index_0' full_phrase='test phrase' line_ids='1,2' />`;
      const result = normalizeCitations(input);
      // Order should be: file_id, start_page_key, full_phrase, line_ids
      const fileIdIndex = result.indexOf("file_id=");
  ...
  describe("multiple citations in text", () => {
    it("normalizes multiple citations in a single string", () => {
      const input = `This is citation 1 <cite file_id='file1' start_page_key='page_1_index_0' full_phrase='first' line_ids='1' /> and citation 2 <cite file_id='file2' start_page_key='page_2_index_0' full_phrase='second' line_ids='2' />.`;
      const result = normalizeCitations(input);
    ...
    it("preserves text between citations", () => {
      const input = `Before <cite file_id='f1' start_page_key='page_1_index_0' full_phrase='a' line_ids='1' /> middle <cite file_id='f2' start_page_key='page_1_index_0' full_phrase='b' line_ids='2' /> after`;
      const result = normalizeCitations(input);
  ...
  describe("quoted and unquoted line_ids/timestamps", () => {
    it("handles unquoted line_ids", () => {
      ...
      const input = `<cite file_id='file1' start_page_key='page_1_index_0' full_phrase='test' line_ids=1,2,3 />`;
      const result = normalizeCitations(input);
    ...
    it("handles double-quoted line_ids", () => {
      const input = `<cite file_id='file1' start_page_key='page_1_index_0' full_phrase='test' line_ids="4,5,6" />`;
      const result = normalizeCitations(input);
    ...
    it("handles line_ids with text labels", () => {
      const input = `<cite file_id='file1' start_page_key='page_1_index_0' full_phrase='test' line_ids='line1,line2,line3' />`;
      const result = normalizeCitations(input);
      expect(result).toContain("line_ids='1,2,3'");
    });
  });
